...
class LlavaOnevisionConfig(PreTrainedConfig):
    ...
    Args:
        ...
        text_config (`Union[Auto-Config, dict]`, *optional*, defaults to `Qwen2Config`):
    ...
    # sub_configs = {"text_config": AutoConfig, "vision_config": AutoConfig} # This is informational, not functional
        ...
        **kwargs,
        ...
        image_grid_pinpoints = (
            ...
            if image_grid_pinpoints is not None
                ...
                [384, 384], [384, 768], [384, 1152], [
                    384, 1536], [384, 1920], [384, 2304],
                [768, 384], [768, 768], [768, 1152], [
                    768, 1536], [768, 1920], [768, 2304],
                [1152, 384], [1152, 768], [1152, 1152], [
                    1152, 1536], [1152, 1920], [1152, 2304],
                [1536, 384], [1536, 768], [1536, 1152], [
                    1536, 1536], [1536, 1920], [1536, 2304],
                [1920, 384], [1920, 768], [1920, 1152], [
                    1920, 1536], [1920, 1920], [1920, 2304],
                [2304, 384], [2304, 768], [2304, 1152], [
                    2304, 1536], [2304, 1920], [2304, 2304],
            ]
        )
        ...
        if isinstance(vision_config, dict):
            ...
            vision_config = CONFIG_MAPPING["siglip_vision_model"](
                hidden_size=1152, intermediate_size=4304, patch_size=14, image_size=384,
                num_hidden_layers=26, num_attention_heads=16, vision_use_head=False,
            )
        self.vision_config = vision_config

        # --- BUG FIX START ---
        # The original code created a default Qwen2Config, ignoring all the
        # language model parameters passed in kwargs (from config.json).
        # This fix ensures that `text_config` is initialized with all the necessary parameters.
        if isinstance(text_config, dict):
            text_config = CONFIG_MAPPING[text_config.get(
                "model_type", "qwen2")](**text_config)
        elif text_config is None:
            # When loading from a pretrained config.json, `text_config` is None,
            # and all parameters are in `kwargs`. We must use them here.
            text_config = CONFIG_MAPPING["qwen2"](**kwargs)
