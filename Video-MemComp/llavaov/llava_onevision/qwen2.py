...
#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
# from transformers import Qwen2RotaryEmbedding
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def _prepare_attention_mask_with_fusion_bias(
        self,
        q_len: int,
        kv_len: int,
        fusion_counts: Optional[torch.Tensor],
        batch_size: int,
        dtype: torch.dtype,
        device: torch.device,
        ...
        past_kv_len = kv_len - q_len

        attn_bias = torch.zeros(
            (batch_size, 1, q_len, kv_len), device=device, dtype=dtype)
        ...
        if q_len > 1:
            ...
            attn_bias[..., :, past_kv_len:].masked_fill_(
                causal_mask_bool, torch.finfo(dtype).min
            )

        if fusion_counts is not None and fusion_counts.numel() > 0:
            if fusion_counts.dim() == 1:
                fusion_counts = fusion_counts.unsqueeze(0)

            if fusion_counts.shape[0] != batch_size:
                fusion_counts = fusion_counts.expand(batch_size, -1)

            fusion_counts = fusion_counts.to(device=device, dtype=dtype)

            attention_correction = torch.log1p(fusion_counts - 1)
            attention_correction = attention_correction.unsqueeze(
                1).unsqueeze(1)

            correction_len = min(kv_len, fusion_counts.shape[-1])
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if layer_fusion_counts is not None:
                ...
                kv_len = past_kv_len + q_len

                fusion_counts_for_layer = layer_fusion_counts[layer_idx]

                attention_mask_for_layer = self._prepare_attention_mask_with_fusion_bias(
                    q_len=q_len, kv_len=kv_len, fusion_counts=fusion_counts_for_layer,
