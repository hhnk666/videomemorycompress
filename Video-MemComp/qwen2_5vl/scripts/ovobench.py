...
if 1:
    from qwen2_5_vl import Qwen2_5_VLForConditionalGeneration, Qwen2_5_VLSdpaAttention
# Parameters
...
def chunk_video(video_path, end_time, start_time=0):
    ...
    output_file = osp.join(
        output_dir, f"{video_name}_{start_time}_{end_time}.mp4")
    ...
    temp_audiofile = osp.join(
        "tmp", f"temp_audio_{video_name}_{start_time}_{end_time}.mp3")
...
def score(results):
    def calculate_score_backward_realtime(results):
        ...
        for i in range(len(results)):
            results[i]["score"] = get_score(
                results[i]["response"], results[i]["ground_truth"])
        ...
        for i, result in enumerate(results):
            ...
            if result["task"] == "REC":
                ...
                for j, test_info_ in enumerate(result["test_info"]):
                    ...
                    cnt_correct += get_score_REC(
                        test_info_["response"], test_info_["count"])
            ...
            if result["task"] == "SSR":
                ...
                for j, test_info_ in enumerate(result["test_info"]):
                    ...
                    cnt_correct += get_score_SSR_CRR(
                        test_info_["response"], gt)
    ...
    if len(backward_results) > 0:
        ...
        backward_results, backward_scores = calculate_score_backward_realtime(
            backward_results)
        ...
        logger.info(
            f"Backward Avg.: {100 * sum(avg_scores['backward'])/len(avg_scores['backward']):.2f}\n")
    else:
        # correct_backward = 0
        # total_backward = 0
        pass
    ...
    if len(realtime_results) > 0:
        ...
        realtime_results, realtime_scores = calculate_score_backward_realtime(
            realtime_results)
        ...
        logger.info(
            f"Realtime Avg.: {100 * sum(avg_scores['realtime'])/len(avg_scores['realtime']):.2f}\n")
    else:
        # correct_realtime = 0
        # total_realtime = 0
        pass
    ...
    if len(forward_results) > 0:
        ...
        forward_results, forward_scores = calculate_score_forward(
            forward_results)
        ...
        logger.info(
            f"Forward Avg.: {100 * sum(avg_scores['forward'])/len(avg_scores['forward']):.2f}\n")
    else:
        # correct_forward = 0
        # total_forward = 0
        pass
...
if __name__ == "__main__":
    ...
    LOG_PATH = osp.join(RESULT_DIR, LOG_PATH.format(
        run_name=RUN_NAME, curr_time=curr_time))
    OUTPUT_JSONL = osp.join(RESULT_DIR, OUTPUT_JSONL.format(
        run_name=RUN_NAME, curr_time=curr_time))
    DR_SAVE_PATH = osp.join(RESULT_DIR, DR_SAVE_PATH.format(
        run_name=RUN_NAME, curr_time=curr_time))
    ...
    for item in tqdm(task_list):
        ...
        try:
            ...
            if item['task'] in backward_tasks or item['task'] in realtime_tasks:
                ...
                chunk_video_path = chunk_video(
                    video_path=video_path, end_time=realtime)
                response = get_response(
                    prompt=prompt, video_path=chunk_video_path, model=model, processor=processor)
    ...
    if DROP_METHOD is not None and DR_SAVE_PATH is not None:
        ...
        logger.info(
            f"Total drop ratio (weighted drop ratio): {100*total_dr:.1f}%")
        logger.info(
            f"Average drop ratio (unweighted drop ratio): {100*avg_dr:.1f}%")
