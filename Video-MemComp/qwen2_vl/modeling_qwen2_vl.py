    ...
    def forward(
        ...
        **kwargs,
        ...
        query_states, key_states, value_states = (
            self.qkv(hidden_states).reshape(seq_length, 3,
                                            self.num_heads, -1).permute(1, 0, 2, 3).unbind(0)
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if position_ids is None:
            position_ids = cache_position.view(
                1, 1, -1).expand(3, inputs_embeds.shape[0], -1)
...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    ...
    def __init__(self, config: Qwen2VLConfig):
        ...
        self.threshold_intra = 0.55
        self.threshold_inter = 0.2
        self.post_init()
    ...
    def _proxy_prefill_and_get_queries(self, past_key_values: Cache):
        ...
        proxy_ids = torch.tensor(
            [[151645, 198, 151644, 77091, 198]], device=device)
        proxy_embeds = self.language_model.embed_tokens(proxy_ids)
        ...
        for layer_idx, layer in enumerate(self.language_model.layers):
            ...
            current_past_len = self.streaming_state["logical_length"]
            ...
            q_rot, _ = apply_multimodal_rotary_pos_emb(
                q_vectors, q_vectors, proxy_pos_embeds[0], proxy_pos_embeds[1],
    ...
    def get_rope_index(
        ...
        if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
            ...
            for i, input_ids in enumerate(total_input_ids):
                input_ids = input_ids[attention_mask[i].to(
                    input_ids.device) == 1]
                ...
                for _ in range(image_nums + video_nums):
                    ...
                    t_index = torch.arange(
                        llm_grid_t).view(-1, 1).expand(-1, llm_grid_h * llm_grid_w).flatten()
        ...
        for layer_idx in range(num_layers):
            ...
            with torch.autocast(device_type=k_layer.device.type, dtype=kv_dtype):
                ...
                if fc_layer.numel() == scores.numel():
                    ...
                    scores = scores + torch.sqrt(attention_bias+1)
    ...
    def _create_streaming_attention_mask(
        ...
        if q_len > 1:
            causal_mask_bool = torch.triu(torch.ones(
        ...
        if combined_fusion_counts is not None and combined_fusion_counts.numel() > 0:
            ...
            if correction_len > 0:
                ...
                attention_correction = torch.log1p(
                    combined_fusion_counts[..., :correction_len] - 1).unsqueeze(0).unsqueeze(0).unsqueeze(0)
    ...
    def forward(
        ...
        rope_deltas: Optional[torch.LongTensor] = None,
        cache_position: Optional[torch.LongTensor] = None,
        drop_threshold: Optional[float] = 0.3,
        drop_threshold_in_frame: Optional[float] = 0.6,
        **kwargs,
    ) -> Union[tuple, Qwen2VLModelOutputWithPast]:
        drop_threshold = self.threshold_inter
        drop_threshold_in_frame = self.threshold_intra
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
        ...
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        print(
            f"drop_threshold: {drop_threshold}, drop_threshold_in_frame: {drop_threshold_in_frame}")
        ...
        if not is_video_input:
            ...
            return Qwen2VLModelOutputWithPast(
                last_hidden_state=outputs.last_hidden_state, past_key_values=outputs.past_key_values,
                hidden_states=outputs.hidden_states, attentions=outputs.attentions, rope_deltas=self.rope_deltas,
            )
        ...
        if pkv is None:
            pkv = DynamicCache(config=self.config.text_config)
        ...
        if not current_fusion_counts:
            current_fusion_counts = [torch.tensor(
                [], device=self.device) for _ in range(len(self.language_model.layers))]
        ...
        for t_id in unique_time_ids:
            ...
            if is_video_frame:
                ...
                if surviving_embeds_inter.shape[1] > 1:
                    ...
                    cosine_similarities = torch.matmul(F.normalize(
                        survivor_tokens), F.normalize(tokens_before_intra_merge).t())
                    gsim_matrix = torch.where(
                        cosine_similarities >= 0.5, cosine_similarities, 0.0)
                    m_matrix = F.softmax(gsim_matrix * 150, dim=0)
                    s_factors = torch.sum(m_matrix, dim=1)

                    processed_fusion_counts = s_factors.float()
                else:
                    processed_embeds = surviving_embeds_inter
                    processed_pos_ids = position_ids[:, :,
                                                     original_frame_indices_after_inter]
            ...
            if processed_embeds.shape[1] > 0:
                all_chunks_to_process.append({
                    "embeds": processed_embeds, "pos_ids": processed_pos_ids, "fusion_counts": processed_fusion_counts
                })
        ...
        for chunk in all_chunks_to_process:
            ...
            self.streaming_state["logical_length"] = chunk["pos_ids"][1].max(
            )+1
            fc_history = self.streaming_state["fusion_counts"]

            vision_pos_ids_chunk = pos_ids_chunk[1:,
                                                 ...] if pos_ids_chunk.shape[0] == 4 else pos_ids_chunk
            pos_embeds_chunk = self.language_model.rotary_emb(
                hidden_states_chunk, vision_pos_ids_chunk)
            for layer_idx, layer in enumerate(self.language_model.layers):

                combined_fc_for_layer = torch.cat(
                    [fc_history[layer_idx], fusion_counts_chunk.to(fc_history[layer_idx].device)])
                ...
                attn_bias = self._create_streaming_attention_mask(
                    ...
                    device=hidden_states_chunk.device, dtype=hidden_states_chunk.dtype
                )

                layer_outputs = layer(
                    hidden_states_chunk, attention_mask=attn_bias, past_key_values=pkv,
                    use_cache=True, position_embeddings=pos_embeds_chunk,
                )
                hidden_states_chunk = layer_outputs[0]

            self.streaming_state["fusion_counts"] = [
                torch.cat([fc, fusion_counts_chunk.to(fc.device)]) for fc in fc_history]
            last_hidden_state_of_sequence = hidden_states_chunk

        final_pkv = self.streaming_state["past_key_values"]
        final_hidden_states = self.language_model.norm(
            last_hidden_state_of_sequence) if last_hidden_state_of_sequence is not None else torch.zeros_like(inputs_embeds[:, :1, :])

        if not return_dict:
            return (final_hidden_states, final_pkv, None, None)

        return Qwen2VLModelOutputWithPast(
            last_hidden_state=final_hidden_states,
            past_key_values=final_pkv,
            hidden_states=None, attentions=None, rope_deltas=self.rope_deltas,
        )
        ...
        if inputs_embeds is not None:
            vision_start_mask = (
                ...
                == self.get_input_embeddings()(
                    torch.tensor(vision_start_token_id,
                                 dtype=torch.long, device=inputs_embeds.device)
                )
            )[..., 0]
            image_mask = (
            ...
            video_mask = (
                inputs_embeds
                == self.get_input_embeddings()(
                    torch.tensor(video_token_id, dtype=torch.long,
                                 device=inputs_embeds.device)
                )
            )[..., 0]
        else:
        ...
        **model_kwargs,
        ...
        def _expand_dict_for_generation_visual(dict_to_expand):
            ...
            image_nums, video_nums = self._get_image_nums_and_video_nums(
                input_ids, inputs_embeds=model_kwargs.get(
                    "inputs_embeds", None)
            )
...
__all__ = ["Qwen2VLForConditionalGeneration", "Qwen2VLModel",
           "Qwen2VLPreTrainedModel", "Qwen2VLTextModel"]
